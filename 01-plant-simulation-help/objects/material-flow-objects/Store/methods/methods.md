# Methods and Read-Only Attributes of the PE (Place in the Store)
...
- `X` (integer): position of the storage place (on the production element) in the X-Dimension.
- `Y` (integer): position of the storage place (on the production element) in the Y-Dimension.
...
- Do not specify the optional parameter `ExitBlockingList` to return an array containing the objects in the Exit Blocking List. The array is one-dimensional and only contains the objects in the blocking list. If you need the start times of the blockages (contained in the blocking table), query them from the object with the attribute `BlockingStarttime`.
...
- `ExitBlockingList` (table, optional): name of the table (a DataTable or a local variable) into which the method writes the values.

  Plant Simulation automatically generates the format of this table with two columns:
...
**See also:** Exit Blocking List, Check the Contents List of the Stations, `BlockingStarttime` [SimTalk], exitBlockList [SimTalk] - material flow objects, exitBlockList [SimTalk] - lane A or B

---

## getStackHeight — `Store`
...
- Here the height of the stack designates its physical height, not the number of parts on the stack.
...
**See also:** Stack Parts in the Store, Length [preferences], X-Dimension [Store], Y-Dimension [Store], `ZDim` [SimTalk] - Store
...
- The last MU is always the MU that is located on the bottom storage place; as a rule this is the MU that was moved onto the object first.
...
**See also:** X-Dimension [Store], Y-Dimension [Store], Z-Dimension [Store], `NumMU` [SimTalk] - PE, Store
