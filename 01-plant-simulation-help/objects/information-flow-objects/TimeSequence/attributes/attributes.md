# Attributes of the TimeSequence
...
```simtalk
MyTimeSequence.ReferenceTime := str_to_time("1:00:00:00.00")
```

- To get the value of an attribute, you might, for example, type:

```simtalk
print MyTimeSequence.Active
...
### Absolute
Sets the Time Reference of the TimeSequence designated by `<Path>` to Absolute (`true`) or Relative (`false`).
...
**See also:** Time Reference [TimeSequence]

### Active
Activates the object designated by `<Path>` (`true`), i.e., makes it record data, or deactivates it (`false`).
...
**See also:** Active [check box] - TimeSequence

### DefaultValue
Sets the Default Value of the TimeSequence designated by `<Path>`.

- **Remarks:** The data type of the TimeSequence matches the data type of the column **Value** on the tab **Contents** of the TimeSequence.
- **Type:** Attribute
- **Syntax:** `<Path>.DefaultValue:any`
...
**See also:** Default Value [TimeSequence]

### Path
Sets the Path to the value which the TimeSequence designated by `<Path>` is to record.

- **Remarks:** The value can be specified as a relative or as an absolute path.
- **Type:** Attribute
...
**See also:** Value [TimeSequence]

### ReferenceDate
Sets the offset value of the time sequence inputs for the setting **Time Reference > Absolute** of the TimeSequence designated by `<Path>`.

- **Remarks:** Plant Simulation adds it to each time value in the TimeSequence table. By changing the reference time, you can shift the values in time without re-entering the data.
- **Type:** Attribute
...
### ReferenceTime
Sets the Start Time of the TimeSequence designated by `<Path>`.
...
### Sample
Makes the TimeSequence designated by `<Path>` record data periodically (`true`) or watch data, i.e., records them only when its value changes (`false`).

- **Type:** Attribute
- **Syntax:** `<Path>.Sample:boolean`
- **Assignment Value:** You can assign a value of data type boolean.
...
**See also:** Mode [drop-down list] - TimeSequence

### ShowColumnIndex
Shows the column index of the TimeSequence designated by `<Path>` (`true`) or hides it (`false`).
...
**See also:** Activate Column Index

### ShowRowIndex
Shows the row index of the TimeSequence designated by `<Path>` (`true`) or hides it (`false`).
...
### SmpPeriod
Sets how often the TimeSequence designated by `<Path>` takes samples, when Sample mode is active.

- **Remarks:** `SmpPeriod` applies when the attribute `Sample` is set to `true`.
- **Type:** Attribute
...
**See also:** Interval [TimeSequence], Sample [SimTalk], Trigger [object]

## Trigger

Use the object **Trigger** for changing values of attributes and global variables according to a pattern you define during a simulation run. It can also activate methods, which then execute the actions you programmed.
...
To show a tooltip with information about the Trigger, hover with the mouse over it.

To change the length of the graphic and the anchor points of the Trigger, click **Show Manipulators** on the Edit ribbon tab or press **M** on the keyboard.
