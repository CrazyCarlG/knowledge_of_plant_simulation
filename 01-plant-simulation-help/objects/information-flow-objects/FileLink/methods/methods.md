...
Opens the file linked to the FileLink designated by `<Path>` in its originating application or the respective file type assigned on the computer. You can then edit it.

### Remarks

If you deactivate the safety setting **File > Model Settings > General > Prohibit Access to the Computer**, the FileLink cannot open the embedded files in the associated application with a double-click. The same applies to the method `openFile`.
...
## Read-Only Attributes of the FileLink

The FileLink provides the Read-Only Attributes of All Objects.
