...
- **Type:** Attribute
- **Syntax:** `<Path>.PredecessorNumber:integer`
...
**See also:** Fluid from Predecessor [text box], DePortioner
...
When the created fluid flows out of the DePortioner into a Pipe, the MU that delivered it will be deleted. If the outflow of fluids is interrupted, caused by a failure, a pause, or because the successor cannot accept anything at the moment, the Exporters will not be released.

You can, for example, use the DePortioner to empty barrels of fluids or bulk goods, which are then used as ingredients for mixed goods down your production line, and feed the material into Pipes.

You can think of the DePortioner as the counterpart of the Portioner.

To show a tooltip with information about the DePortioner, hover with the mouse over it. To change the length of the graphic and the anchor points of the Portioner, click **Show Manipulators** on the Edit ribbon tab or press `M` on the keyboard.

### Add the Object to the Simulation Model

To add the object DePortioner to your simulation model, click **Manage Class Library > Basic Objects > Fluids > DePortioner** on the Home ribbon tab.
...
**See also:** Portion and Deportion Materials, Configure the DePortioner, Dialog Box of the DePortioner

---

## Dialog Box of the DePortioner

Double-click the icon of the DePortioner to open its dialog box.
...
### Outflow Rate [DePortioner]

Type the **Outflow Rate** into the text box. This is the rate with which the material flows out of the DePortioner after it has emptied the arriving part and flows through the objects of type Pipe to the next object in the flow of materials.

**Remarks:** The Outflow Rate is the amount of liters of the material that flows off in a second.

**Note:** The current Outflow Rate depends on the number of attached Pipes. If you attached two Pipes, the specified Outflow Rate flows through each one of these Pipes in case the Outflow Rate of the connected Pipes enables this. If you just want to let the specified amount flow out of the object, attach a single Pipe and split that up into several Pipes later on.

**SimTalk:** `OutflowRate [SimTalk] - DePortioner`

### Fluid Depends On

Select how the DePortioner defines the material and the amount of the fluid that is to be created.

You can select one of these settings:

- **Fixed** — Set the material and the amount per MU in the dialog items *Material* and *Amount per MU*.
- **MU Name** — Set the material and the amount per MU in the *Mapping Table*.
- **MU Attribute** — Set the material and the amount per MU in the dialog items *Material Attribute* and *Amount Attribute*. These are user-defined attributes of the MU which you have to create.

Define the name of each material that is to be used in the *MaterialsTable*.

**SimTalk:** `FluidDependsOn [SimTalk]`

### Material [DePortioner]

Type the name of the Material into the text box, which the DePortioner creates.

**Remarks:** It applies if you select *Fixed* for the setting *Fluid Depends On*.

**SimTalk:** `Material [SimTalk] - DePortioner`

### Amount per MU [DePortioner]

Type the total amount in liters of the fluid into the text box, which the DePortioner creates.

**Remarks:** *Amount per MU* applies if you select *Fixed* for the setting *Fluid Depends On*.

**SimTalk:** `AmountPerMU [SimTalk] - Portioner`

### Mapping Table

Type the name of the Mapping Table into the text box.

**Remarks:** The Mapping Table contains the MU Name of the arriving MU, and the Material and the Amount of the fluid which the DePortioner creates. The Mapping Table applies if you select *MU Name* for the setting *Fluid Depends On*.

You can also click and select the Mapping Table.

**SimTalk:** `MappingTable [SimTalk]`

### Material Attribute

Type the name of the user-defined Material Attribute into the text box. It defines the fluid which the DePortioner creates.

**Remarks:** The Material Attribute applies if you select *MU Attribute* for the setting *Fluid Depends On*.

**SimTalk:** `AttrNameMaterial [SimTalk]`

### Amount Attribute

Type the name of the user-defined Amount Attribute into the text box, which sets the amount of the fluid which the DePortioner creates.

**Remarks:** The Amount Attribute applies if you select *MU Attribute* for the setting *Fluid Depends On*.

**SimTalk:** `AttrNameAmount [SimTalk]`

### Materials Table [DePortioner]

The MaterialsTable contains the data of the different materials which the DePortioner can create.

**Remarks:** Click the ellipsis button and select the MaterialsTable in the dialog *Select Object*. Or select the MaterialsTable in a Frame, drag it to the text box and drop it there.

**SimTalk:** `MaterialsTable [SimTalk] - DePortioner`

**See also:** MaterialsTable, Select Object [for controls]

### Current Amount [DePortioner]

Shows the Current Amount of the fluid that is located in the DePortioner at the moment.

**SimTalk:** `CurrentAmount [SimTalk] - DePortioner`

### Current Outflow Rate [DePortioner]

Shows the Current Outflow Rate, i.e., the amount of liters of the fluid which flows out of the DePortioner in a second.
...
**See also:** Recovery Time [general description], Set-up Time [general description], Select the Set-Up Time, `RecoveryTime [SimTalk] - Mixer`, `SetupTime [SimTalk] - fluid objects`
...
**See also:** Set a Station Up, Set-up Only When Empty, Set-up For, Set-up After n Parts, Automatic Set-up, Set-up Depends On
...
**See also:** Select Object [for controls], Before Actions, Entrance Control [general description], Exit Control [general description], Set-up Control, Pull Control [general description], Shift Calendar [tab Controls]
