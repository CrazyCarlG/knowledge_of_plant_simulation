# Read-Only Attributes of the FootPath

## Overview

The Exporter provides the **Read-Only Attributes of All Objects**.

You can query the values of read-only attributes, but you cannot set them: Plant Simulation computes the value for the point-in-time at which you query it. In most cases a read-only attribute corresponds to an unavailable dialog item on one of the object's tabs (for example, the **Statistics** tab).
...
- If a parameter has a default value, the signature shows it after the parameter, for example `:= false`.
