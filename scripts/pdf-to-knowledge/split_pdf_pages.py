...
2) 基本命令：
    python scripts/split_pdf_pages.py "源PDF路径.pdf" 起始页 结束页 "目标路径"
...
4) 示例（目标路径是目录）：
    python scripts/split_pdf_pages.py \
      ...
      "C:/Users/you/Desktop/output"

    生成文件名：manual_10-25.pdf
...
def build_output_path(
    ...
    if target_path.suffix.lower() == ".pdf":
        return target_path
    return target_path / output_name


def validate_args(pdf_path: Path, start_page: int, end_page: int) -> int:
    """在打开 PDF 前，做基础参数校验。"""
...
def main() -> int:
    ...
    validation_error = validate_args(pdf_path, start_page, end_page)
    if validation_error:
    ...
    output_path = build_output_path(pdf_path, start_page, end_page, target_path)
